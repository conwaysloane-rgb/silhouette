...
export default function ListingCard({ listing }: Props) {
  ...
  return (
    <Link href={`/browse/${listing.id}`} className="group block">
      ...
      {/* Info */}
      <p className="text-sm font-semibold text-neutral-900 truncate leading-tight">{listing.title}</p>
      <div className="flex items-center justify-between mt-0.5">
        <span className="text-xs text-neutral-500">{CATEGORY_LABELS[listing.category]} · {listing.size}</span>
        <span className="text-xs font-semibold text-neutral-900">${listing.price_per_day}/day</span>
      </div>
      {listing.seller && (
        <p className="text-[11px] text-neutral-400 mt-0.5 truncate">{listing.seller.full_name}</p>
      )}
    </Link>
  )
}
